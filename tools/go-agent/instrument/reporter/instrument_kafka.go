...
type KafkaInstrument struct {
	hasToEnhance bool
	compileOpts  *api.CompileOptions
}

func NewKafkaInstrument() *KafkaInstrument {
	return &KafkaInstrument{}
}

func (i *KafkaInstrument) CouldHandle(opts *api.CompileOptions) bool {
	i.compileOpts = opts
	return opts.Package == "github.com/apache/skywalking-go/agent/reporter"
}

func (i *KafkaInstrument) FilterAndEdit(path string, curFile *dst.File, cursor *dstutil.Cursor, allFiles []*dst.File) bool {
	if i.hasToEnhance {
		return false
	}
	i.hasToEnhance = true
...
func (i *KafkaInstrument) AfterEnhanceFile(fromPath, newPath string) error {
	return nil
}

func (i *KafkaInstrument) WriteExtraFiles(dir string) ([]string, error) {
	...
	reporterDirName := filepath.Join("reporter", "kafka")
...
func (i *KafkaInstrument) generateReporterInitFile(dir string) (string, error) {
	return tools.WriteFile(dir, "kafka_init.go", html.UnescapeString(tools.ExecuteTemplate(`package reporter
...
`, struct {
		...
		InitFuncName: consts.KafkaInitFuncName,
